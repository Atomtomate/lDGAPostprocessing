...
jldopen(dataPath*"/ED_out_dbg.jld2", "w") do f
    f["Γch"] = Γch
    f["Γsp"] = Γsp
    f["χupup"] = permutedims(reshape(TwoPartGF_upup, 2*nFermi, 2*nFermi, 2*nBose+1),[3,2,1])
    f["χupdo"] = permutedims(reshape(TwoPartGF_updo, 2*nFermi, 2*nFermi, 2*nBose+1),[3,2,1])
